...
class RefreshToken {
    ...
    public static function revokeByHash(string $tokenHash): void {
        Database::execute('UPDATE refresh_tokens SET revoked = 1 WHERE token_hash = ?', [$tokenHash]);
    }

    public static function revokeAllByUser(int $userId): void {
        Database::execute('UPDATE refresh_tokens SET revoked = 1 WHERE user_id = ?', [$userId]);
    }
}
?>